...
def _make_car_polygon(front=0.95, rear=0.658, half_w=0.75):
    ...
    pts = [
        (front, 0),
        (front - 0.20, nose_w), (front - 0.40, nose_w + 0.05),
        (front - 0.15, half_w), (front - 0.25, half_w),
        (front * 0.35, rw + 0.08), (0.0, rw + 0.05),
        (-0.05, half_w * 0.65), (-rear * 0.3, half_w * 0.62),
        (-rear * 0.7, half_w * 0.58), (-rear, half_w * 0.55),
        (-rear - 0.05, half_w), (-rear - 0.12, half_w),
        (-rear - 0.12, -half_w), (-rear - 0.05, -half_w),
        (-rear, -half_w * 0.55), (-rear * 0.7, -half_w * 0.58),
        (-rear * 0.3, -half_w * 0.62), (-0.05, -half_w * 0.65),
        (0.0, -(rw + 0.05)), (front * 0.35, -(rw + 0.08)),
        (front - 0.25, -half_w), (front - 0.15, -half_w),
        (front - 0.40, -(nose_w + 0.05)), (front - 0.20, -nose_w),
    ]
    return np.array(pts)
...
def _compute_viz_data(sol, params):
    ...
    return {
        "path_xy": path_xy, "vehicle_heading": headings + psi_err,
        "v": v, "s": s, "delta": delta,
...
def _setup_tall_pair(ax, left_label, right_label, left_color, right_color,
                     ylim, title, centered=True):
...
def _update_tall_pair(val_l, val_r, bar_l, bar_r, txt_l, txt_r, centered):
    ...
    if centered:
        txt_l.set_y(val_l); txt_l.set_va("bottom" if val_l >= 0 else "top")
        txt_r.set_y(val_r); txt_r.set_va("bottom" if val_r >= 0 else "top")
    else:
        txt_l.set_y(val_l); txt_l.set_va("bottom")
        txt_r.set_y(val_r); txt_r.set_va("bottom")
...
def animate_lap(solution_path, output_path, stride=3, fps=20, track_csv=None):
    ...
    wheel_locals = [_make_wheel_rect(l_f, a_l), _make_wheel_rect(l_f, -a_r),
                    _make_wheel_rect(-l_r, -a_r), _make_wheel_rect(-l_r, a_l)]
    ...
    if cones_left is not None:
        ax_track.scatter(cones_left[:, 0], cones_left[:, 1],
                         s=8, color="#2980b9", marker="^", alpha=0.6, zorder=2)
    if cones_right is not None:
        ax_track.scatter(cones_right[:, 0], cones_right[:, 1],
                         s=8, color="#f39c12", marker="^", alpha=0.6, zorder=2)
    ...
    # ── Tall bar panels ──
    fx_f = _setup_tall_pair(ax_fx_f, "FL", "FR", "tab:blue", "tab:orange",
                            Fx_max, "Fx Front [N]", centered=True)
    fx_r = _setup_tall_pair(ax_fx_r, "RL", "RR", "tab:red", "tab:green",
                            Fx_max, "Fx Rear [N]", centered=True)
    fz_f = _setup_tall_pair(ax_fz_f, "FL", "FR", "tab:blue", "tab:orange",
                            Fz_max, "Fz Front [N]", centered=False)
    fz_r = _setup_tall_pair(ax_fz_r, "RL", "RR", "tab:red", "tab:green",
                            Fz_max, "Fz Rear [N]", centered=False)

    # ── Steering wheels ──
    ax_steer.set_xlim(-2.2, 2.2); ax_steer.set_ylim(-2.0, 1.4)
    ax_steer.set_aspect("equal")
    ax_steer.set_title("Steering", fontsize=9, pad=2)
    ax_steer.set_xticks([]); ax_steer.set_yticks([])
    ax_steer.plot([-1.2, 1.2], [0, 0], color="#bbb", lw=2, zorder=1)

    sw_w, sw_h = 0.35, 0.9
    steer_wl = mpatches.FancyBboxPatch((-sw_w/2, -sw_h/2), sw_w, sw_h,
        boxstyle="round,pad=0.05", fc="#333", ec="#111", lw=1, zorder=5)
    steer_wl.set_transform(Affine2D().translate(-0.9, 0) + ax_steer.transData)
    ax_steer.add_patch(steer_wl)

    steer_wr = mpatches.FancyBboxPatch((-sw_w/2, -sw_h/2), sw_w, sw_h,
        boxstyle="round,pad=0.05", fc="#333", ec="#111", lw=1, zorder=5)
    steer_wr.set_transform(Affine2D().translate(0.9, 0) + ax_steer.transData)
    ax_steer.add_patch(steer_wr)

    steer_text = ax_steer.text(0, -1.6, "", ha="center", va="top", fontsize=9,
                                fontweight="bold")

    # ── TV yaw moment (vertical bar) ──
    ax_tv.set_xlim(0, 1); ax_tv.set_ylim(-Mz_max, Mz_max)
    ax_tv.axhline(0, color="#bbb", lw=0.5)
    ax_tv.set_title("TV [Nm]", fontsize=9, pad=2)
    ax_tv.set_xticks([]); ax_tv.tick_params(axis="y", labelsize=6)
    ...
    # ── Speed (vertical bar) ──
    ax_speed.set_xlim(0, 1); ax_speed.set_ylim(0, v_max)
    ax_speed.set_title("Speed", fontsize=9, pad=2)
    ax_speed.set_xticks([]); ax_speed.tick_params(axis="y", labelsize=6)
    speed_bar = ax_speed.bar(0.5, 0, width=0.6, color="#e74c3c", alpha=0.8)[0]
    speed_text = ax_speed.text(0.5, 0, "", ha="center", va="bottom", fontsize=8,
...
if __name__ == "__main__":
    ...
    # Fast: stride=4, 24fps
    animate_lap(sol_path, out_dir / "lap_animation_fast.gif",
                stride=4, fps=24, track_csv=csv_path)

    # Slow: stride=1, 12fps
    animate_lap(sol_path, out_dir / "lap_animation_slow.gif",
                stride=1, fps=12, track_csv=csv_path)
