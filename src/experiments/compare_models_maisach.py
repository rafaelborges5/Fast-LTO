...
from optimization.integrators import EulerIntegrator
from vehicle_models import DynamicBicycleModel, FourWheelModel, PointMassModel
...
def make_four_wheel_no_aero():
    return FourWheelModel(params={
        "C_l": 0.0, "C_d": 0.0, "C_r": 0.0,
...
def make_dynamic_bicycle():
    return DynamicBicycleModel(params={
        "m": 160.0, "Iz": 250.0,
        "lf": 0.872, "lr": 0.658, "g": 9.81,
        "Bf": 9.0, "Cf": 1.3, "Dmf_f": 1.2,
        "Br": 9.0, "Cr": 1.3, "Dmf_r": 1.2,
        "mu": 1.2, "gamma_ellipse": 1.0,
        "use_friction_ellipse": True,
        "v_min": 0.4, "v_max": 20.0,
        "v_lat_max": 4.0, "yaw_rate_max": 3.0,
        "a_long_min": -15.0, "a_long_max": 15.0,
        "delta_max": 0.4,
        "d_max": 3.0, "psi_err_max": 1.2,
        "eps_s_dot": 1.0, "eps_D_kappa": 0.05,
        "v_eps": 0.5, "smoothmax_eps": 1e-3,
...
def _solve_one(name, model, track_data, sol_path, reg_du, reg_u_l2):
    ...
    try:
        sol = solve_ocp_and_save(
            track=track_data, model=model, solution_path=sol_path,
            integrator=EulerIntegrator(), initial_speed=5.0,
            reg_du=reg_du, reg_u_l2=reg_u_l2,
            use_normalization=True, solver_verbose=False,
...
def run_experiment(ds_m: float = 1.0):
    ...
    from utils.track_bounds import compute_lateral_bounds, load_boundaries, save_track_with_widths
    ...
    track_obj = fit_and_discretize(csv_path, ds_m=ds_m, continuity="C2", viz=False, save_path=disc_path)
    boundaries = load_boundaries(csv_path)
    result = compute_lateral_bounds(track_obj, left=boundaries["left"], right=boundaries["right"])
    save_track_with_widths(widths_path, track=track_obj, csv_source=csv_path, result=result, bounds_config={})
    ...
    print(f"\n{'#'*60}")
    print(f"  SCENARIO A: FAIR COMPARISON (no aero, static loads)")
    ...
    fair["point_mass"] = _solve_one(
        "point_mass", make_point_mass(), track_data,
        SOLUTIONS_DIR / f"maisach_fair_point_mass_{tag}.json", 600.0, None)
    fair["dynamic_bicycle"] = _solve_one(
        "dynamic_bicycle", make_dynamic_bicycle(), track_data,
        SOLUTIONS_DIR / f"maisach_fair_dynamic_bicycle_{tag}.json", 600.0, None)
    fair["four_wheel_no_aero"] = _solve_one(
        "four_wheel (no aero)", make_four_wheel_no_aero(), track_data,
        SOLUTIONS_DIR / f"maisach_fair_four_wheel_no_aero_{tag}.json", 0.0, 2.0)

    # ---- Scenario B: Full (with aero) ----
    print(f"\n{'#'*60}")
    print(f"  SCENARIO B: FULL FOUR-WHEEL (with aero + load transfer)")
    print(f"{'#'*60}")

    full = {}
    full["four_wheel_full"] = _solve_one(
        "four_wheel (full aero)", make_four_wheel_full(), track_data,
        SOLUTIONS_DIR / f"maisach_full_four_wheel_{tag}.json", 0.0, 2.0)
