...
def _compute_four_wheel_forces(
    *,
    ...
    if mode == "static":
        ...
        alpha_fl = np.arctan2(vy_front, np.maximum(np.abs(vx_fl), 0.3) * np.sign(np.where(vx_fl == 0, 1.0, vx_fl))) - delta
        alpha_fr = np.arctan2(vy_front, np.maximum(np.abs(vx_fr), 0.3) * np.sign(np.where(vx_fr == 0, 1.0, vx_fr))) - delta
        alpha_rr = np.arctan2(vy_rear, np.maximum(np.abs(vx_rr), 0.3) * np.sign(np.where(vx_rr == 0, 1.0, vx_rr)))
        alpha_rl = np.arctan2(vy_rear, np.maximum(np.abs(vx_rl), 0.3) * np.sign(np.where(vx_rl == 0, 1.0, vx_rl)))

        def _pacejka(alpha, B, C, D):
            return D * np.sin(C * np.arctan(B * alpha))

        f_fl = _pacejka(alpha_fl, float(params.get("B_fl", 9.0)), float(params.get("C_fl", 1.3)), float(params.get("D_fl", 1.2)))
        f_fr = _pacejka(alpha_fr, float(params.get("B_fr", 9.0)), float(params.get("C_fr", 1.3)), float(params.get("D_fr", 1.2)))
        f_rr = _pacejka(alpha_rr, float(params.get("B_rr", 9.0)), float(params.get("C_rr", 1.3)), float(params.get("D_rr", 1.2)))
        f_rl = _pacejka(alpha_rl, float(params.get("B_rl", 9.0)), float(params.get("C_rl", 1.3)), float(params.get("D_rl", 1.2)))
        ...
        for i in range(N):
            A = np.array([
                [1 + f_fl[i] * (h1 * sd[i] - h2 * cd[i]),
                 f_fr[i] * (h1 * sd[i] - h2 * cd[i]), 0, 0],
                [f_fl[i] * (h1 * sd[i] + h2 * cd[i]),
                 1 + f_fr[i] * (h1 * sd[i] + h2 * cd[i]), 0, 0],
                [-h1 * sd[i] * f_fl[i], -h1 * sd[i] * f_fr[i],
                 1 + h2 * f_rr[i], h2 * f_rl[i]],
                [-h1 * sd[i] * f_fl[i], -h1 * sd[i] * f_fr[i],
                 -h2 * f_rr[i], 1 - h2 * f_rl[i]],
            ])
    ...
    f_fl_vis = _pacejka_np(alpha_fl, float(params.get("B_fl", 9.0)), float(params.get("C_fl", 1.3)), float(params.get("D_fl", 1.2)))
    f_fr_vis = _pacejka_np(alpha_fr, float(params.get("B_fr", 9.0)), float(params.get("C_fr", 1.3)), float(params.get("D_fr", 1.2)))
    f_rr_vis = _pacejka_np(alpha_rr, float(params.get("B_rr", 9.0)), float(params.get("C_rr", 1.3)), float(params.get("D_rr", 1.2)))
    f_rl_vis = _pacejka_np(alpha_rl, float(params.get("B_rl", 9.0)), float(params.get("C_rl", 1.3)), float(params.get("D_rl", 1.2)))
    ...
    return {
        "alpha_fl": alpha_fl, "alpha_fr": alpha_fr,
        "alpha_rr": alpha_rr, "alpha_rl": alpha_rl,
        "Fy_fl": Fy_fl, "Fy_fr": Fy_fr,
        "Fy_rr": Fy_rr, "Fy_rl": Fy_rl,
        "Fz_fl": Fz_fl, "Fz_fr": Fz_fr,
        "Fz_rr": Fz_rr, "Fz_rl": Fz_rl,
        "util_fl": util_fl, "util_fr": util_fr,
        "util_rr": util_rr, "util_rl": util_rl,
        "F_drag": F_drag, "F_roll": F_roll,
    }
...
def _compute_constraint_activity_four_wheel(
    *,
    d, w_left, w_right, v,
    Fx_fl, Fx_fr, Fx_rr, Fx_rl,
    util_fl, util_fr, util_rr, util_rl,
    params,
):
    n = len(d)
    if n == 0:
    ...
    tol_v = 0.5
    v_active = float(np.count_nonzero(
        (np.abs(v - v_min) < tol_v) | (np.abs(v - v_max) < tol_v)
    ) / n)
...
def plot_all_panels_four_wheel(
    ...
    forces = _compute_four_wheel_forces(
        v=v, v_lat=v_lat, yaw_rate=yaw_rate,
    ...
    a_x, a_y = _compute_body_accels(
        Fx_fl=Fx_fl, Fx_fr=Fx_fr, Fx_rr=Fx_rr, Fx_rl=Fx_rl,
        Fy_fl=forces["Fy_fl"], Fy_fr=forces["Fy_fr"],
        Fy_rr=forces["Fy_rr"], Fy_rl=forces["Fy_rl"],
        delta=delta, F_drag=forces["F_drag"], F_roll=forces["F_roll"],
        params=params,
    )

    constraint_activity = _compute_constraint_activity_four_wheel(
        d=d, w_left=w_left, w_right=w_right, v=v,
        Fx_fl=Fx_fl, Fx_fr=Fx_fr, Fx_rr=Fx_rr, Fx_rl=Fx_rl,
        util_fl=forces["util_fl"], util_fr=forces["util_fr"],
        util_rr=forces["util_rr"], util_rl=forces["util_rl"],
        params=params,
    )
    ...
    # 1. Track + speed
    plot_path_with_speed(cones_left, cones_right, path_xy, v,
                         out_path=None, show=False, fig=fig, ax=axes[0])
...
def plot_diagnostics_four_wheel(
    *,
    ...
    for name, key in [("FL", "alpha_fl"), ("FR", "alpha_fr"),
                      ("RR", "alpha_rr"), ("RL", "alpha_rl")]:
        axes[1].plot(s, np.degrees(forces[key]), color=WHEEL_COLORS[name],
    ...
    for name, key in [("FL", "util_fl"), ("FR", "util_fr"),
                      ("RR", "util_rr"), ("RL", "util_rl")]:
        axes[4].plot(s, forces[key], color=WHEEL_COLORS[name],
                     label=name, linewidth=0.8)
    ...
    # 6. Vertical loads
    for name, key in [("FL", "Fz_fl"), ("FR", "Fz_fr"),
                      ("RR", "Fz_rr"), ("RL", "Fz_rl")]:
        axes[5].plot(s, forces[key], color=WHEEL_COLORS[name],
                     label=name, linewidth=0.8)
    ...
    # 7. Lateral forces
    for name, key in [("FL", "Fy_fl"), ("FR", "Fy_fr"),
                      ("RR", "Fy_rr"), ("RL", "Fy_rl")]:
        axes[6].plot(s, forces[key], color=WHEEL_COLORS[name],
                     label=name, linewidth=0.8)
