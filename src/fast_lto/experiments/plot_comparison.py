...
MODELS_FAIR = {
    "point_mass": {
        "file": "maisach_fair_point_mass",
        "label": "Point Mass",
        "color": "tab:green",
        "ls": "-",
    },
    "dynamic_bicycle": {
        "file": "maisach_fair_dynamic_bicycle",
        "label": "Dyn. Bicycle",
        "color": "tab:orange",
        "ls": "-",
    },
    "four_wheel_no_aero": {
        "file": "maisach_fair_four_wheel_no_aero",
        "label": "4W (no aero)",
        "color": "tab:blue",
        "ls": "-",
    },
    "four_wheel_full": {
        "file": "maisach_full_four_wheel",
        "label": "4W (full aero)",
        "color": "tab:red",
        "ls": "--",
    },
...
def main(ds: float = 1.0):
    ...
    for k, cfg in MODELS_FAIR.items():
        s = np.array(data[k]["arc_lengths"])
        ax.plot(
            s, np.array(data[k]["d"]), color=cfg["color"], ls=cfg["ls"], lw=0.9, label=cfg["label"]
        )
    ax.set_xlabel("s [m]")
    ax.set_ylabel("d [m]")
    ax.legend(fontsize=8)
    ...
    for k, cfg in MODELS_FAIR.items():
        ...
        if "a_lat" in d:
            ...
            vl = np.array(d["v_lat"])
            Fx_fl = np.array(d["Fx_fl"])
            Fx_fr = np.array(d["Fx_fr"])
            Fx_rr = np.array(d["Fx_rr"])
            Fx_rl = np.array(d["Fx_rl"])
            delta = np.array(d["delta"])
            cd = np.cos(delta)
            C_d_v = float(params.get("C_d", 0))
            C_r_v = float(params.get("C_r", 0))
    ...
    lines = [
        ...
        "Matched: m=160kg, D=1.2, v_max=20 m/s",
        "4 corner constraints + 15cm margin",
        "",
        ...
        "",
        "Note: PM and Bicycle have no aero drag",
        "in dynamics -> optimistic at high speed.",
    ]
    ax.text(
        0.0,
        1.0,
